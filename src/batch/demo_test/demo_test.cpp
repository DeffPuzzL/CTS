...
void    CDemo::vDemoPrint()
{
    ...
    IBPrace("当前交易流水(%d), 接入PID(%d), lNo(%d), (%.6f)", pstCts->m_lSeqNo, pstCts->m_lInPid, lNo, dAmt);
...
//    if(RC_FAIL == (lSeqNo = lNewTrade(pstSavm, pstCts, "100000", true)))
    if(RC_FAIL == (lSeqNo = lNewTrade(pstSavm, pstCts, "100000", false)))
    {
        SYSError(RET_SYSTEM, "发起新交易(10000)失败"); 
        return ;
    }
