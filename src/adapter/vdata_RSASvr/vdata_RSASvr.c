...
#define        TAIL_CRLF                   "\r\n"
#define        HTTP_TAIL                   "\r\n\r\n"
#define        PACK_SIGN                   "&sign="
#define        LENG_SIGN                   6	// "strlen("&sign=")

#define        ASYN_RESP                   "retCode=010&retMsg=交易失败"
...
void    vHttpReturn(BSock skSock, char *pszBuffer, long lLen)
{
	...
	char		*p = pszBuffer + lLen + LENG_SIGN;
	...
	{
        pstHttp->m_lBody = lGetUserSize() - lLen - LENG_SIGN;	
        if(RC_SUCC != lPackRSAsign(pszBuffer, lLen, p, &pstHttp->m_lBody))
        {
            IBPerror("组织发送报文字符串签名失败");
        	return ;
        }
        
        memcpy(pszBuffer + lLen, PACK_SIGN, LENG_SIGN);
        pstHttp->m_lRecv = lLen + LENG_SIGN + pstHttp->m_lBody;
        pstHttp->m_lBody = snprintf(pstHttp->m_szBuffer, sizeof(pstHttp->m_szBuffer), 
            "HTTP/1.1 200 OK\r\nCache-Control: no-cache\r\nContent-Type: text/plain; charset=UTF-8"
            "\r\nConnection: close\r\nContent-Length: %d\r\n\r\n%s", pstHttp->m_lRecv, pszBuffer);
	}
...
//	IBPrace("收到请求报文(%d):%s", pv->m_lRecv, pv->m_pvData);
	pv->m_lRecv = lUrldecode(pv->m_pvData, pv->m_lRecv, pv->m_pvData, pv->m_lRecv);
//	IBPrace("UrlDecode(%d):%s", pv->m_lRecv, pv->m_pvData);
	if(NULL == (p = strstr(pv->m_pvData, PACK_SIGN)) || p == pv->m_pvData)
	{
		lRet = snprintf(szBuffer, sizeof(szBuffer), "retCode=010&retMsg=请求报文异常");
        vHttpReturn(pc->m_skSock, szBuffer, lRet);
        goto HTTP_ERROR;
	}

	lRet = p - pv->m_pvData;
	pv->m_lRecv = pv->m_lRecv - lRet - LENG_SIGN;
	if(RC_SUCC != lPackRSAverify(pv->m_pvData, lRet, p + LENG_SIGN, pv->m_lRecv))
	{
		lRet = snprintf(szBuffer, sizeof(szBuffer), "retCode=010&retMsg=验签失败");
        vHttpReturn(pc->m_skSock, szBuffer, lRet);
        goto HTTP_ERROR;
	}

//	IBPrace("验签通过 compelete successfully!!");
    if(g_stHost.m_bAsyn)
    {
        lAsPutRequest(pstSavm, szBuffer, lRet, pc->m_skSock, pv->m_szCode, "");
		lRet = strlen(strncpy(szBuffer, ASYN_RESP, sizeof(szBuffer)));
        vHttpReturn(pc->m_skSock, szBuffer, lRet);
        memset(pv, 0, sizeof(long) * 5);
        return RC_CLOSE;
    }

    if(RC_SUCC != lInPutRequest(pstSavm, pv->m_pvData, lRet, pc->m_skSock, pv->m_szCode, ""))
    {
		lRet = snprintf(szBuffer, sizeof(szBuffer), "retCode=%s&retMsg=%s", sGetRetCode(), 
			sGetRetMsg());
        vHttpReturn(pc->m_skSock, szBuffer, lRet);
        goto HTTP_ERROR;
    }
...
long    lProcInitail(SATvm *pstSavm, void *arg)
{
    pthread_t   tPid = 0;
    long        lHost = *((long *)arg);
	char		szPvCert[512], szPuCert[512];
	...
	snprintf(szPuCert, sizeof(szPuCert), "%s/%s", getenv("CERTDIR"), g_stHost.m_szAuthUser);
	snprintf(szPvCert, sizeof(szPvCert), "%s/%s", getenv("CERTDIR"), g_stHost.m_szAuthPwd);
