...
//#ifdef		CODING_GBK
//		对汉子可能有影响, 汉子的一半可能是a - z之间
		if((uchar)s[i] > 0x80 && 0 == fg)   fg = 1; //  GBK汉子字节
		else								fg = 0;
//#endif
	}
	return s;
...
char*	sGetValueByIdx(char *p, char *s, long lIdx)
{
	...
	for(i = 0; i < m; i ++)
	{
		...
		if((uchar)p[i] > 0x80 && 0 == fg)   fg = 1; //  GBK汉子字节
		else								fg = 0;
	}

	if(ldx + 1 < lIdx) return szOut;
...
long	lStringReplace(char *s, char *o, char *d, char **pp)
{
	...
	if(!s || !o || !d || !strlen(o) || !strlen(d))  return RC_FAIL;
